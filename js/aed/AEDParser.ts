...
interface AedData {
    columns: AedColumn[]
    metadata: Record<string, Record<string, { type: string; value: string }>>
}

interface AedFeatureData {
    aed: AedData
    allColumns: string[]
    chr: string | null
    start: number | null
    ...
    name: string | null
    color: string | null
    popupData: () => Array<{ name: string; value: string }>
}

interface DataWrapper {
    nextLine(): Promise<string | undefined> | string | undefined
}
...
type HeaderProperties = Record<string, any>
...
class AEDParser {
    ...
    skipRows: number
    decode: (this: AEDParser, tokens: string[], wig: any) => AedFeatureData | undefined
    delimiter: string
    header: HeaderProperties | undefined
    format: string | undefined
    aed: AedData | undefined

    constructor(config?: { decode?: (tokens: string[], wig: any) => AedFeatureData | undefined; nameField?: string }) {
    ...
    async parseHeader(dataWrapper: DataWrapper): Promise<HeaderProperties | undefined> {
        let line: string | undefined
        let header: HeaderProperties | undefined
    ...
    async parseFeatures(dataWrapper: DataWrapper): Promise<AedFeatureData[]> {

        const allFeatures: AedFeatureData[] = []
        let cnt: number = 0
        ...
        while ((line = dataWrapper.nextLine() as string | undefined) !== undefined) {
            i++
        ...
        function readTokensAed(): string[] {
            var tokens: string[] = [],
                token: string = "",
                quotedString: boolean = false,
                n: number,
                c: string
            ...
            while (line || line === '') {
                ...
                if (quotedString) {
                    ...
                    line = undefined // Note: original code calls nextLine() but it's not available here
                } else {
                    // We can end the loop
                    break
                }
...
function parseAedToken(value: string): AedColumn {
...
function parseAedHeaderRow(tokens: string[]): AedData {
...
function parseTrackLine(line: string): TrackProperties {
...
function parseColumnsDirective(line: string): Record<string, any> {
...
function decodeBedGraph(this: AEDParser, tokens: string[], ignore: any): any {

    var chr: string, start: number, end: number, value: number
    ...
    const feature: any = {chr: chr, start: start, end: end, value: value}
...
function parseAttributeString(attributeString: string, keyValueDelim: string): Record<string, string> {
 ...
 */
function AedFeature(this: AedFeatureData, aed: AedData, allColumns: string[]): void {
    var token: string | number, aedColumn: AedColumn, aedColumns: AedColumn[] = aed.columns
    ...
    for (let i = 0; i < allColumns.length; i++) {
        ...
        if (aedColumn.namespace.trim() === 'bio') {
            if (aedColumn.name === 'sequence') {
                this.chr = token as string
            } else if (aedColumn.name === 'start') {
                this.start = token as number
            } else if (aedColumn.name === 'end') {
                this.end = token as number
            } else if (aedColumn.name === 'cdsMin') {
                this.cdStart = token as number
            } else if (aedColumn.name === 'cdsMax') {
                this.cdEnd = token as number
            } else if (aedColumn.name === 'strand') {
                this.strand = token as string
            }
        } else if (aedColumn.namespace === 'aed') {
            if (aedColumn.name === 'name') {
                this.name = token as string
            }
        } else if (aedColumn.namespace === 'style') {
            if (aedColumn.name === 'color') {
                this.color = IGVColor.createColorString(token as string)
            }
        }
    }
}

(AedFeature as any).prototype.popupData = function (this: AedFeatureData): Array<{ name: string; value: string }> {
    var data: Array<{ name: string; value: string }> = [],
        aed = this.aed
 ...
 */
function decodeAed(this: AEDParser, tokens: string[], ignore: any): AedFeatureData | undefined {
    var name: string | undefined, value: string | undefined, token: string,
        nonEmptyTokens: number = 0,
        aedColumns: AedColumn[] = this.aed!.columns,
        aedColumn: AedColumn,
        aedKey: AedColumn,
        i: number
    ...
    var feature = new (AedFeature as any)(this.aed, tokens) as AedFeatureData
