...
class AlignmentTrack extends TrackBase {
    ...
    constructor(config: TrackConfig, browser: Browser) {
        ...
        if (config.highlightedReads) {
            this.setHighlightedReads(config.highlightedReads, undefined)
        }
    ...
    setHighlightedReads(highlightedReads: string[], highlightColor: string | undefined) {
        ...
        if (size > 0) {
            ...
            if (size > 1) {
                for (let m of this._baseModifications) {
                    let key =
                        menuItems.push(this.basemodColorByCB({
                            key: 'basemod:' + m,
                            label: `base modification (${modificationName(m as string)})`
                        }))
                }
            }
            ...
            if (size > 1) {
                for (let m of this._baseModifications) {
                    menuItems.push(this.basemodColorByCB({
                        key: 'basemod2:' + m,
                        label: `base modification 2-color (${modificationName(m as string)})`
                    }))
                }
            }
        }
    ...
    basemodColorByCB(menuItem: {key: string, label: string}) {
...
function shadedBaseColor(qual: number, baseColor: string) {
