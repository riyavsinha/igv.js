...
const BamUtils = {
    ...
    bam_tag2cigar: function (ba: Uint8Array, block_end: number, seq_offset: number, lseq: number, al: BamAlignment, cigarArray: CigarOp[]): boolean {

        function type2size(x: string): number {
    ...
    decodeBamRecords: function (ba: Uint8Array, offset: number, alignmentContainer: any[], chrNames: string[], chrIdx: number | undefined, min: number, max: number, filter?: any): boolean | undefined {
        ...
        while (offset < ba.length) {
            ...
            let readName: string[] | string = []
            for (let j = 0; j < nl - 1; ++j) {
                (readName as string[]).push(String.fromCharCode(ba[offset + 36 + j]))
            }
            readName = (readName as string[]).join('')

            let lengthOnRef = 0
            let cigar = ''
            let p = offset + 36 + nl
            const cigarArray: CigarOp[] = []

            let lastCigRecord: CigarOp | undefined
            // NOTE: mOperators is constructed but never used -- the commented-out code below references it.
            ...
            alignment.readName = readName as string
            ...
            let seq: string[] | string = []
            const seqBytes = (lseq + 1) >> 1
            for (let j = 0; j < seqBytes; ++j) {
                var sb = ba[p + j]
                ;(seq as string[]).push(SEQ_DECODER[(sb & 0xf0) >> 4])
                ;(seq as string[]).push(SEQ_DECODER[(sb & 0x0f)])
            }
            seq = (seq as string[]).slice(0, lseq).join('')  // seq might have one extra character (if lseq is an odd number)
            p += seqBytes


            const qualArray: number[] = []
    ...
    decodeSamRecords: function (sam: string, alignmentContainer: any[], chr: string, min: number, max: number, filter?: any): void {
        ...
        for (i = 0; i < len; i++) {
            ...
            cigarArray.forEach(function (op: CigarOp) {
                var opLen = op.len
                var opLtr = op.ltr
                if (opLtr === 'M' || opLtr === 'EQ' || opLtr === 'X' || opLtr === 'D' || opLtr === 'N' || opLtr === '=')
                    lengthOnRef += opLen
            ...
            for (j = 0; j < qualString.length; j++) {
                (alignment.qual as number[])[j] = qualString.charCodeAt(j) - 33
            }
    ...
    setPairOrientation: function (alignment: BamAlignment): void {
 ...
 */
function makeBlocks(alignment: BamAlignment, cigarArray: CigarOp[]): void {
 ...
 * @return
 */
function buildOperators(cigarString: string): CigarOp[] {

    var operators: CigarOp[], buffer: string[], i: number, len: number, prevOp: CigarOp | null, next: string, op: string, nBases: number
...
function decodeSamTags(tags: string[]): Record<string, string> {
...
function decodeBamTags(ba: Uint8Array): Record<string, any> {
    ...
    while (p < len) {
        ...
        if (type === 'A') {
            ...
            value = dataView.getInt8(p)
            p++
        } else if (type === 'C') {
            value = dataView.getUint8(p)
            p++
