...
interface CSIBinIndex {
    [binNumber: number]: [VirtualOffset, VirtualOffset][]
}

interface CSIRefIndex {
    binIndex: CSIBinIndex
    loffset: (VirtualOffset | null)[]
}

async function parseCsiIndex(arrayBuffer: ArrayBuffer): Promise<CSIIndex> {
...
class CSIIndex {
    ...
    parse(arrayBuffer: ArrayBuffer): void {
        ...
        for (let ref = 0; ref < nref; ref++) {
            const binIndex: CSIBinIndex = {}
            const loffset: (VirtualOffset | null)[] = []
    ...
    chunksForRange(refId: number, min: number, max: number): Chunk[] {
        ...
        if (!ba) {
            ...
            return optimizeChunks(chunks, lowestOffset ?? undefined)
        }
    }

    getParentBin(bin: number): number {
        if (bin == 0) {
            return 0
        }
        return (bin - 1) >> 3
