...
class SegParser {
    ...
    async parseHeader(dataWrapper: DataWrapper): Promise<SegHeader> {
    ...
    async parseFeatures(dataWrapper: DataWrapper): Promise<SegFeature[]> {
        ...
        while ((line = await dataWrapper.nextLine()) !== undefined) {
            ...
            if (tokens.length > this.dataColumn!) {
                ...
                if (extraHeaders) {
                    const extraValues: string[] = this.extractExtraColumns(tokens)
    ...
    extractExtraColumns(tokens: string[]): string[] {
        const extras: string[] = []
...
class SegFeature {
    ...
    setAttributes({names, values}: SegAttributeParams): void {
    ...
    popupData(type: string, genomeID: string): (PopupDataEntry | string)[] {
    ...
    extractCravatLink(genomeId: string): string | undefined {

        let ref: string | undefined, alt: string | undefined
        if (this.attributeNames && this.attributeNames.length > 0) {
            for (let i = 0; i < this.attributeNames.length; i++) {
