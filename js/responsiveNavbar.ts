...
class ResponsiveNavbar {
    ...
    navbarDidResize(): void {
        ...
        let navbarButtonClass: string | undefined
        const threshold = 8
    ...
    updateLocus(loc: string, chrName: string): void {
        if (this.searchInput) {
    ...
    currentNavbarButtonClass(): string {
        return this.currentClass
    }

    setEnableTrackSelection(b: boolean): void {
        this.multiTrackSelectButton.setMultiTrackSelection(b)
    }

    getEnableTrackSelection(): boolean {
    ...
    async doSearch(locus: string): Promise<void> {
