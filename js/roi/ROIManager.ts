...
class ROIManager {
    ...
    constructor(browser: any) {
        ...
        this.roiTable = new ROITable(browser)
        this.top = 0
        this.roiSets = []
    ...
    async repaintTable(): Promise<void> {
        const records = await this.getTableRecords()
        this.roiTable.renderTable(records)
    }
    ...
    setROITableButtonVisibility(isVisible: boolean): void {
    ...
    async renderAllROISets(): Promise<void> {
    ...
    async renderROISet({browser, pixelTop, roiSet}: { browser: any, pixelTop: number, roiSet: ROISet }): Promise<void> {
        ...
        for (let i = 0; i < columns.length; i++) {
            ...
            for (let el of elements) {
                const regionKey = (el as HTMLElement).dataset.region!
            ...
            if (features) {
                ...
                for (let feature of features) {
                    ...
                    if (el) {
                        (el as HTMLElement).style.left = `${pixelX}px`;
                        (el as HTMLElement).style.width = `${pixelWidth}px`
    ...
    renderSVGContext(columnContainer: HTMLElement, context: any, {deltaX, deltaY}: { deltaX: number, deltaY: number }): void {
        ...
        for (const regionElement of columnContainer.querySelectorAll('.igv-roi-region')) {
            ...
            // body
            const {x, y, width, height} = (regionElement as HTMLElement).getBoundingClientRect()
            context.fillStyle = (regionElement as HTMLElement).style.backgroundColor
            context.fillRect(x - deltaX, y + deltaY, width, height)

            // header
            const header = regionElement.querySelector('div') as HTMLElement
    ...
    getUserDefinedROISet(): ROISet | undefined {
        return this.roiSets.find(roiSet => true === roiSet.isUserDefined)
    }

    deleteUserDefinedROISet(): void {
    ...
    async deleteRegionWithKey(regionKey: string, columnContainer: HTMLElement): Promise<void> {
...
function locusChangeHandler(this: ROIManager): void {
    this.renderAllROISets()
}

function createSelector(regionKey: string): string {
