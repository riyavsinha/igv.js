...
class ROITable extends RegionTableBase {
    ...
    copySequenceButton!: HTMLElement
    toggleROIButton!: HTMLElement
    boundCopySequenceButtonHandler!: (event: Event) => void
    ...
    renderTable(records: ROIRecord[]): void {

        Array.from(this.tableRowContainer.querySelectorAll('.igv-roi-table-row')).forEach((el: Element) => el.remove())

        if (records.length > 0) {

            const sortedRecords = records.sort((a: ROIRecord, b: ROIRecord) => (a.feature.chr.localeCompare(b.feature.chr) || a.feature.start - b.feature.start || a.feature.end - b.feature.end))
    ...
    set footerDOM(gotoButtonHandler: (event: Event) => void) {
        ...
        async function copySequenceButtonHandler(this: ROITable, event: Event): Promise<void> {
            ...
            if (selected.length > 0) {
                ...
                for (let el of selected) {
                    const { locus } = parseRegionKey((el as HTMLElement).dataset.region!)
                    loci.push(locus)
                }
        ...
        function toggleROIButtonHandler(this: ROITable, event: Event): void {
            event.preventDefault()
            event.stopPropagation()
    ...
    static getColumnFormatConfiguration(doIncludeROISetNames: boolean): ColumnFormat[] {
    ...
    static gotoButtonHandler(this: ROITable, event: Event): void {
...
function enableButton(button: HTMLElement, doEnable: boolean): void {
