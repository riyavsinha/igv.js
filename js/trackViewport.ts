...
class TrackViewport extends Viewport {
    ...
    spinnerElement: HTMLDivElement | undefined
    overlayElement: HTMLDivElement | undefined
    zoomInNoticeElement: HTMLElement | undefined
    ...
    didPresentZoomInNotice(): boolean {
    ...
    checkZoomIn(): boolean {
        ...
        const zoomedOutOfWindow = (): boolean => {
    ...
    setHeight(h: number): void {
        super.setHeight(h)
        const labelElement = this.viewportElement.querySelector(".igv-track-label") as HTMLElement | null
        if (labelElement) {
    ...
    async loadFeatures(): Promise<FeatureCache | undefined> {
    ...
    repaintDimensions(): { bpStart: number; bpEnd: number; pixelWidth: number } {
    ...
    containsPosition(chr: string, position: number): boolean {
    ...
    static createZoomInNotice(parentElement: HTMLElement): HTMLElement {
    ...
    addMouseHandlers(): void {
        ...
        // Mouse down
        const md = (event: MouseEvent | TouchEvent) => {
        ...
        // Mouse up
        const mu = (event: MouseEvent | TouchEvent) => {
        ...
        if (typeof this.trackView.track.hoverText === 'function') {
            ...
            this.viewportElement.addEventListener('mousemove', ((event: MouseEvent) => {
                const now = Date.now()
    ...
    addViewportContextMenuHandler(viewport: HTMLElement): void {
    ...
    addTrackLabelClickHandler(trackLabel: HTMLElement): void {
        trackLabel.addEventListener('click', (event: MouseEvent) => {
            event.stopPropagation()
    ...
    async handleTrackClick(event: MouseEvent): Promise<string | undefined> {
...
class FeatureCache {
    ...
    overlapsRange(chr: string, start: number, end: number): boolean {
