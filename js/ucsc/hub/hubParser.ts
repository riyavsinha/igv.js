...
async function loadStanzas(url: string): Promise<Stanza[]> {
    ...
    const nodes: Stanza[] = []
    let currentNode: Stanza | undefined
...
function resolveParents(nodes: Stanza[]): Stanza[] {
    const nodeMap = new Map<string, Stanza>()
    for (let n of nodes) {
...
function getDataURL(url: string, host: string, baseURL: string): string {
