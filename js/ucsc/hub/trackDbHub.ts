...
const vizModeMap = new Map<string, string>([
    ["pack", "EXPANDED"],
    ["full", "EXPANDED"],
    ["squish", "SQUISHED"],
    ["dense", "COLLAPSED"]
])
const typeFormatMap = new Map<string, string>([
    ["vcftabix", "vcf"],
    ["vcfphasedtrio", "vcf"],
    ["bigdbsnp", "bigbed"],
    ["genepred", "refgene"]
...
class TrackDbHub {
    ...
    getGroupedTrackConfigurations(): TrackConfigContainer[] {

        if (!this.groupTrackConfigs) {
            this.groupTrackConfigs = []
            const trackContainers = new Map<string, TrackConfigContainer>()
...
function getPriority(priorityString: string): number {
    try {
